...
def get_control_360_data():
    """Datos completos para Control 360° con información detallada de rutas"""
    return {
        'hub': {
            'name': 'AIFA',
            'lat': 19.7373,
...
def render_control_360_tab():
    """Renderiza el tab del Control 360° con componentes Dash nativos"""
    data = get_control_360_data()
    routes = data['routes']
    hub = data['hub']
    
    # Calcular métricas dinámicas
    total_routes = len(routes)
    total_pax = sum(route['pax'] for route in routes) // 1000000
    total_revenue = sum(route['revenue'] for route in routes)
    intl_routes = len([r for r in routes if r['type'] == 'international'])
    domestic_routes = len([r for r in routes if r['type'] == 'domestic'])
    ...
    return html.Div([
        ...
        # Grid principal: Panel de control + Mapa 3D
        dbc.Row([
            ...
            dbc.Col([
                ...
                dbc.Card([
                    dbc.CardHeader([
                        html.H5([
                            DashIconify(icon="mdi:flight-takeoff", width=20, style={"marginRight": "8px"}),
                            "Estado del Sistema"
                        ], className="mb-0", style={"color": "var(--primary-cyan)"})
                    ]),
                    dbc.CardBody([
                        # Indicador de status
                        html.Div([
                            html.Div(style={
                                ...
                                "background": "var(--success-green)", "marginRight": "8px",
                                "animation": "pulse 2s infinite"
                            }),
                            html.Span("Sistema Operacional", style={"color": "var(--text-primary)"})
                        ...
                        dbc.Row([
                            dbc.Col([
                                html.Div([
                                    html.Div(str(total_routes), 
                                            style={"color": "var(--primary-cyan)", "fontSize": "1.5rem", 
                                                  "fontWeight": "700", "marginBottom": "0.25rem"}),
                                    html.Div("Rutas Activas", 
                ...
                ], className="mb-3", style={"background": "var(--glass-bg)", "border": "1px solid var(--glass-border)"}),
                
                # Filtros de Red
                dbc.Card([
                    dbc.CardHeader([
                        html.H5([
                            DashIconify(icon="mdi:tune", width=20, style={"marginRight": "8px"}),
                            "Filtros de Red"
                        ], className="mb-0", style={"color": "var(--primary-cyan)"})
                    ]),
                    dbc.CardBody([
                        html.Label("Tipo de Ruta", style={"color": "var(--text-secondary)", "fontSize": "0.9rem", "marginBottom": "0.5rem"}),
                        dbc.ButtonGroup([
                            dbc.Button([
                                DashIconify(icon="mdi:earth", width=14, style={"marginRight": "6px"}),
                                f"Todas ({total_routes})"
                            ], id="btn-all-routes", color="primary", outline=True, size="sm", className="mb-2"),
                            dbc.Button([
                                DashIconify(icon="mdi:airplane", width=14, style={"marginRight": "6px"}),
                                f"Internacionales ({intl_routes})"
                            ], id="btn-intl-routes", color="primary", outline=True, size="sm", className="mb-2"),
                            dbc.Button([
                                DashIconify(icon="mdi:home", width=14, style={"marginRight": "6px"}),
                                f"Domésticas ({domestic_routes})"
                            ], id="btn-dom-routes", color="primary", outline=True, size="sm", className="mb-2")
                ...
                dbc.Card([
                    dbc.CardHeader([
                        html.H5([
                            DashIconify(icon="mdi:chart-line", width=20, style={"marginRight": "8px"}),
                            "Métricas Live"
                        ], className="mb-0", style={"color": "var(--primary-cyan)"})
                    ]),
                    dbc.CardBody([
                        html.Div([
                            html.Div(f"${total_revenue}M", 
                                    style={"color": "var(--primary-cyan)", "fontSize": "1.5rem", 
                                          "fontWeight": "700", "marginBottom": "0.25rem"}),
                            html.Div("Ingresos Anuales", 
                                    style={"color": "var(--text-secondary)", "fontSize": "0.8rem"})
                        ], style={"textAlign": "center", "padding": "1rem", "marginBottom": "1rem",
                                "background": "rgba(255,255,255,0.04)", "borderRadius": "8px"}),
                        ...
                        dbc.Row([
                            dbc.Col([
                                html.Div([
                                    html.Div("84.5%", 
                                            style={"color": "var(--primary-cyan)", "fontSize": "1.2rem", 
                                                  "fontWeight": "700", "marginBottom": "0.25rem"}),
                                    html.Div("Load Factor", 
                ...
                dbc.Card([
                    dbc.CardHeader([
                        dbc.Row([
                            dbc.Col([
                                html.H5([
                                    DashIconify(icon="mdi:earth", width=20, style={"marginRight": "8px"}),
                            ...
                            dbc.Col([
                                dbc.ButtonGroup([
                                    dbc.Button([
                                        DashIconify(icon="mdi:play", width=14, style={"marginRight": "4px"}),
                                        "Animar"
                                    ], id="btn-animate", color="info", outline=True, size="sm"),
                                    dbc.Button([
                                        DashIconify(icon="mdi:camera", width=14, style={"marginRight": "4px"}),
                                        "Capturar"
                                    ], id="btn-screenshot", color="info", outline=True, size="sm"),
                                    dbc.Button([
                                        DashIconify(icon="mdi:fullscreen", width=14, style={"marginRight": "4px"}),
                                        "Fullscreen"
                                    ], id="btn-fullscreen", color="info", outline=True, size="sm")
                                ], size="sm")
                            ], width="auto")
...
# Control 360° Globe callback
@callback(
    Output('globe-3d-graph', 'figure'),
    [Input('btn-all-routes', 'n_clicks'),
     Input('btn-intl-routes', 'n_clicks'), 
     Input('btn-dom-routes', 'n_clicks')]
)
def update_globe_3d(all_clicks, intl_clicks, dom_clicks):
    """Genera el mapa 3D interactivo con rutas AIFA"""
    try:
        data = get_control_360_data()
        hub = data['hub']
        routes = data['routes']
        ...
        if ctx.triggered:
            button_id = ctx.triggered[0]['prop_id'].split('.')[0]
            if button_id == 'btn-intl-routes':
                filter_type = 'international'
            elif button_id == 'btn-dom-routes':
                filter_type = 'domestic'
        ...
        # 1. Hub AIFA (centro con marcador elegante)
        fig.add_trace(go.Scattergeo(
            lat=[hub['lat']],
            lon=[hub['lon']],
            text=['AIFA HUB'],
            ...
            marker=dict(
                size=35,
                color='#f59e0b',
                line=dict(width=2, color='rgba(255,255,255,0.8)'),  # Contorno semi-transparente
                symbol='star',
                opacity=1.0
            ),
            textposition='bottom center',
        ...
        for route in routes:
            line_color = '#00d4ff' if route['type'] == 'international' else '#00ff88'
            if route['type'] == 'international':
                line_width = max(2, min(route['pax'] / 20000, 8))  # Más sutil: 2-8px
            else:
                line_width = max(1.5, min(route['pax'] / 25000, 6))  # Más sutil: 1.5-6px
            ...
            fig.add_trace(go.Scattergeo(
                ...
                line=dict(
                    width=line_width,
                    color=line_color,
                    opacity=0.7  # Más sutil que 0.9
                ),
                showlegend=False,
                hoverinfo='skip'
            ))
        
        # 3. Marcadores de Destinos Premium  
        for route in routes:
            marker_size = max(20, min(route['pax'] / 6000, 40))
            
            # Color por rentabilidad
            if route['profitability'] == 'high':
                marker_color = '#00ff88'
            elif route['profitability'] == 'medium':
                marker_color = '#f59e0b'
            else:
                marker_color = '#ff4757'
            
            marker_symbol = 'square' if route['type'] == 'international' else 'circle'
            ...
            fig.add_trace(go.Scattergeo(
                ...
                marker=dict(
                    size=marker_size,
                    color=marker_color,
                    line=dict(width=1.5, color='rgba(255,255,255,0.6)'),  # Contorno sutil semi-transparente
                    symbol=marker_symbol,
                    opacity=1.0
                ),
                textposition='top center',
                ...
                hovertemplate='<b>▶ %{text}</b><br>' +
                             '◆ Pasajeros: %{customdata[0]:,}/año<br>' +
                             '◆ Vuelos: %{customdata[1]}/mes<br>' +
                             '◆ Tipo: %{customdata[2]}<br>' +
                             '◆ Revenue: $%{customdata[3]}M MXN<br>' +
                             '◆ Rentabilidad: %{customdata[4]}<extra></extra>',
                customdata=[[route['pax'], route['flights'], 
                           route['type'].title(), route['revenue'], 
                           route['profitability'].upper()]]
            ))
        ...
        print(f"Error en update_globe_3d: {str(e)}")
        # Fallback: mapa básico
        return go.Figure().add_trace(go.Scattergeo()).update_layout(
